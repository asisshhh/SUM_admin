...
export default function HomeHealthcarePackagesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["home-healthcare-packages", filters],
    queryFn: async () => {
      const params = Object.fromEntries(
        Object.entries(filters).filter(([_, v]) => v !== "" && v !== "all")
      );
      return (await api.get("/home-healthcare-packages", { params })).data;
    }
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/home-healthcare-packages/${id}`),
    onSuccess: () => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Manage home healthcare packages and pricing
            </p>
          </div>
        </div>
        {canCreate && (
