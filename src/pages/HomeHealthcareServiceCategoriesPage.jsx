...
function CategoryFormModal({ category, onClose }) {
  ...
  const save = useMutation({
    mutationFn: async (data) => {
      if (isNew) {
        return (await api.post("/home-healthcare-service-categories", data))
          .data;
      }
      return (
        await api.put(
          `/home-healthcare-service-categories/${category.id}`,
          data
        )
      ).data;
    },
 ...
 * MAIN PAGE
 ***************************************************************/
export default function HomeHealthcareServiceCategoriesPage() {
  const qc = useQueryClient();
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["home-healthcare-service-categories", filters],
    queryFn: async () => {
      const params = Object.fromEntries(
        Object.entries(filters).filter(([_, v]) => v !== "" && v !== "all")
      );
      return (await api.get("/home-healthcare-service-categories", { params }))
        .data;
    }
  });
  ...
  const del = useMutation({
    mutationFn: async (id) =>
      (await api.delete(`/home-healthcare-service-categories/${id}`)).data,
    onSuccess: () => {
      qc.invalidateQueries({
        queryKey: ["home-healthcare-service-categories"]
      });
  ...
  const handleDelete = async (cat) => {
    if (cat.serviceCount > 0) {
      alert(
        `Cannot delete category with ${cat.serviceCount} linked services. Remove or reassign services first.`
      );
      return;
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Organize home healthcare services into categories
            </p>
          </div>
        </div>
        {canCreate && (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b">
              <tr>
                ...
                <th className="p-3 text-center font-semibold">Services</th>
