// TermsOfUsePage.jsx — Admin page to edit terms of use
...
export default function TermsOfUsePage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      ...
      if (!isSuperAdmin && !isAdmin) {
        toast.error(
          "Access denied. Only administrators can manage terms of use."
        );
        navigate("/");
      }
    }
  ...
  const updateMutation = useMutation({
    mutationFn: async (newContent) => {
      const response = await api.put("/terms-of-use", {
        content: newContent
      });
      return response.data;
    },
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["terms-of-use"] });
    },
    onError: (err) => {
      toast.error(err.response?.data?.error || "Failed to update terms of use");
    }
  });

  const handleSave = () => {
    if (!content.trim()) {
      toast.error("Terms of use content cannot be empty");
      return;
    }
        ...
        <div className="bg-white rounded-xl border border-red-200 shadow-lg p-8 max-w-md">
          <div className="text-center">
            ...
            <p className="text-slate-600 mb-4">
              Only administrators can manage the terms of use. Please contact
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md">
          <p className="text-red-700">
            Failed to load terms of use. Please try again.
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Terms of Use</h1>
            <p className="text-sm text-slate-500 mt-1">
              Manage and update the terms of use content
            </p>
          </div>
        </div>
        <button
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-purple-700">
              This content will be displayed on the public terms of use page.
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6">
          ...
          <textarea
            ...
            placeholder="Enter terms of use content here. You can use HTML formatting..."
